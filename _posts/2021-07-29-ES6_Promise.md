---
title: 學習 ECMAScript/JavaScript 6 - Promise
...
ECMAScript 6 的 *Promise* 就是一種跳躍語法(goto)，只是不會執行它這一行就立即跳躍，而是延遲到程式區塊結束才跳躍。 Promise 是延遲式跳躍。
...
雖然 executor 最後指派 x 之值為 2 ，
但在呼叫 resolve 時已經保存了 x 當時的狀態 (1)，
所以 then 區塊顯示 x 值是 1 。

## 串接實現非同步工作的同步化

在實務設計上，很多非同步工作實際上有順序依賴性。
比如說 B 工作需要 A 工作的結果才可以執行，也就是 B 必須等 A 跑完才行。
